...
### 인증 — `AuthController` (WMS 현행)

- 경로: `POST /api/auth/login`, `SecurityConfig`에서 `permitAll`.
- 요청 body(Map): `userid`, `password` (필수). 응답 `data`: `accessToken`, `refreshToken`, `userid`, `username`, `usergroupcode`.
- 비즈니스 로직·`adm_userinfo` 조회는 `AuthService` + `AdmUserinfoMapper` (`be-service-mapper` 참고).
...
@Tag(name = "인증", description = "JWT 발급 (adm_userinfo 조회·BCrypt 검증)")
public class AuthController {
    ...
    @Operation(
            summary = "로그인",
            description =
                    "요청 body(Map): userid, password (필수). 사용자는 wms.adm_userinfo 기준. "
...
> WMS 교육 프로젝트 기본 `SwaggerConfig`에는 `GroupedOpenApi` Bean이 없을 수 있습니다. `application.yml`의 `springdoc.packages-to-scan`으로 컨트롤러를 스캔합니다.
