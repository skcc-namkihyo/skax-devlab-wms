...
> **ElMessage / ElMessageBox**: `index.html`에서 `window`에 붙이지 않는다. 각 ES 모듈(`app.js`, `composables`, `views`, `components`) 파일 상단에서 `const { ElMessage } = ElementPlus` 또는 `const { ElMessage, ElMessageBox } = ElementPlus` 로 구조 분해 — 프로젝트 규칙: `frontend.dev.mdc` 4절.
...
// 전역 에러 핸들러 (createApp 이후에 설정) — 상단 const { ElMessage } = ElementPlus 참고
...
- [ ] 프로젝트 디렉토리 생성
- [ ] index.html 생성 및 CDN 링크 확인 (`ElMessage`는 각 모듈에서 `ElementPlus` 구조 분해 — `frontend.dev.mdc` 4절)
- [ ] app.js 생성 (Vue 앱 초기화)
- [ ] router.js 생성 (라우팅 설정)
- [ ] api.js 생성 (Axios 설정)
- [ ] store.js 생성 (전역 상태)
